...
class AgentFactory:
  ...
  ##Agents here are the ghost's
  ##Either agent is red or isn't (how red and blue agents are assigned)
  def __init__(self, isRed, **args):
    self.isRed = isRed

  ##Remember that the agent indices are the following:
  ##Agents 0 & 2 -> Red Team
  ##Agents 1 & 3 -> Blue Team
  def getAgent(self, index):
    "Returns the agent for the provided index."
    util.raiseNotDefined()

##Agent that does random moves while abiing by the rules
...
##Agent that will capture the pellets
class CaptureAgent(Agent):
  ...
  def __init__( self, index, timeForComputing = .5 ):
    """
